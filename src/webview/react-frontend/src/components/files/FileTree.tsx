...
export const FileTree: React.FC<FileTreeProps> = ({ nodes, onOpen, onLongPress, className, viewMode = 'detailed' }) => {
  return (
    <div className={cn(viewMode === 'compact' ? "space-y-0" : "space-y-0.5", className)}>
      <VirtualList
        ...
        itemKey={(n) => n.path}
        renderItem={(n) => (
          <FileNodeItem node={n} onOpen={onOpen} onLongPress={onLongPress} viewMode={viewMode} />
        )}
        className="space-y-0"
      />
    </div>
