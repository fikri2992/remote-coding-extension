    ...
    <div className="bg-card p-4 rounded-lg shadow-sm border border-border">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <button className="rounded-md border border-border px-3 py-2 text-sm hover:bg-muted" onClick={() => { setOutput(''); }}>Clear</button>
          {!fallbackExec && (
            <button className="rounded-md border border-border px-3 py-2 text-sm hover:bg-muted" onClick={() => { activeSessionRef.current = null; ensureSession(); }}>New Session</button>
          )}
        </div>
      </div>
      {fallbackExec ? (
