...
export const CommitComposer: React.FC<{ onCommit: (msg: string) => void; disabled?: boolean; className?: string }>
  ...
  return (
    <div className={`rounded-md border border-border p-3 neo:rounded-none neo:border-[5px] neo:shadow-[6px_6px_0_0_rgba(0,0,0,1)] dark:neo:shadow-[6px_6px_0_0_rgba(255,255,255,0.35)] ${className || ''}`}>
      ...
      <div className="mt-2 flex items-center justify-end">
        <button
          ...
          onClick={submit}
          className="rounded-md bg-primary disabled:opacity-50 px-3 py-2 text-white text-sm hover:bg-primary/90 neo:rounded-none neo:border-[4px] neo:border-border neo:text-primary-foreground neo:shadow-[5px_5px_0_0_rgba(0,0,0,1)] dark:neo:shadow-[5px_5px_0_0_rgba(255,255,255,0.35)]"
        >Commit</button>
      </div>
    </div>
  )
